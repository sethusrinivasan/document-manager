...
object BackupRestore {
    ...
    fun inspectBackup(context: Context, zipFile: File, password: String? = null): BackupInspection {
        ...
        extractDir.mkdirs()
        val pin = password?.trim()?.takeIf { it.isNotEmpty() }

        try {
            val zip = if (pin != null) ZipFile(zipFile, pin.toCharArray()) else ZipFile(zipFile)
            if (zip.isEncrypted && pin == null) {
                return BackupInspection(false, 0, "", 0, 0, true, emptyList(), "Backup is password-protected. Please provide the password.")
            }
            ...
            if (!manifestFile.exists()) {
                ...
                return BackupInspection(false, 0, "", 0, 0, false, emptyList(), "No manifest.json found in backup. Invalid backup file.")
            }
            ...
            val dbCheck = if (backupDb.exists()) RoomDbFiles.inspect(backupDb) else null
            extractDir.deleteRecursively()
            if (dbCheck == null) {
                return BackupInspection(false, schema, timestamp, fileCount, totalSize, encrypted, files,
                    "Backup has no database/traveldocs.db")
            }
            if (!dbCheck.hasDocumentsTable) {
                return BackupInspection(false, schema, timestamp, fileCount, totalSize, encrypted, files,
                    "Backup database is empty or corrupt (no documents table). ${dbCheck.error ?: "tables=${dbCheck.tables}"}")
            }
            DebugLogger.i("Restore", "Inspection: $fileCount files, ${totalSize/1024}KB, docs=${dbCheck.documentCount}")
            return BackupInspection(true, schema, timestamp, fileCount, totalSize, encrypted, files)

        } catch (e: Exception) {
            extractDir.deleteRecursively()
            DebugLogger.e("Restore", "Inspection failed", e)
            return BackupInspection(false, 0, "", 0, 0, false, emptyList(), "Failed to read backup: ${e.message}")
        }
    }
    ...
    fun restoreFromZip(context: Context, zipFile: File, password: String? = null, restoreTag: String = "Restored", onProgress: ((String) -> Unit)? = null): RestoreResult {
        ...
        try {
            ...
            for (i in 0 until filesArray.length()) {
                val f = filesArray.getJSONObject(i)
                val entry = FileEntry(f.getString("path"), f.optLong("size", 0), f.optString("sha256", ""))
            ...
            val backupDb = File(extractDir, "database/traveldocs.db")
            val backupCheck = if (backupDb.exists()) RoomDbFiles.inspect(backupDb) else null
            if (backupCheck == null) {
                extractDir.deleteRecursively()
                reportLines.add("Database: NOT found in backup")
                return RestoreResult(false, 0, 0, 0, "Backup has no database.", reportLines.joinToString("\n"))
            }
            reportLines.add("Backup DB: ${backupDb.length()} bytes, tables=${backupCheck.tables}, docs=${backupCheck.documentCount}, v=${backupCheck.userVersion}")
            if (!backupCheck.hasDocumentsTable) {
                extractDir.deleteRecursively()
                reportLines.add("ERROR: documents table not in backup DB — live library was not changed")
                DebugLogger.e("Restore", "Backup DB missing documents table: ${backupCheck.error ?: backupCheck.tables}")
                return RestoreResult(
                    false, 0, 0, 0,
                    "Backup database is empty or corrupt (no documents table). Restore did not change this phone.",
                    reportLines.joinToString("\n")
                )
            }

            val targetDb = RoomDbFiles.liveFile(context)
            val rollback = File(context.cacheDir, "restore_rollback_${System.currentTimeMillis()}.db")
            fun rollbackLiveDb() {
                if (!rollback.exists()) return
                rollback.copyTo(targetDb, overwrite = true)
                RoomDbFiles.deleteSidecars(targetDb)
                reportLines.add("Rolled back to the previous database")
            }

            RoomDbFiles.closeLiveDatabase(context)
            if (targetDb.exists()) {
                try {
                    targetDb.copyTo(rollback, overwrite = true)
                } catch (e: Exception) {
                    DebugLogger.w("Restore", "Could not snapshot live DB for rollback: ${e.message}")
                }
            }
            RoomDbFiles.deleteSidecars(targetDb)
            ...
            val liveCheck = RoomDbFiles.inspect(targetDb)
            reportLines.add("Live DB after copy: tables=${liveCheck.tables}, docs=${liveCheck.documentCount}")
            if (!liveCheck.hasDocumentsTable) {
                rollbackLiveDb()
                extractDir.deleteRecursively()
                rollback.delete()
                return RestoreResult(
                    false, 0, 0, 0,
                    "Restore failed: documents table missing after copy. Previous library was put back.",
                    reportLines.joinToString("\n")
                )
            }
            reportLines.add("Database: restored (${targetDb.length() / 1024}KB, ${liveCheck.documentCount} documents)")
                    ...
                    try {
                        ...
                        if (hashOk) {
                            ...
                            if (encrypted != null) {
                                ...
                                filesRestored++
                                reportLines.add("  OK: $relativePath (${fileBytes.size/1024}KB)")
                            } else {
            ...
            val docsMissingOnDisk = filesProcessed > 0 && filesRestored == 0
            val failed = filesFailedVerification > 0 || docsMissingOnDisk
            if (failed) {
                rollbackLiveDb()
                rollback.delete()
                val msg = "Restore failed: $filesRestored of $filesProcessed files written" +
                    (if (filesFailedVerification > 0) ", $filesFailedVerification failed verification" else "")
                DebugLogger.e("Restore", msg)
