...
interface DocumentDao {
    ...
    @Query("UPDATE documents SET originalFileName = :originalFileName, updatedAt = :updatedAt WHERE id = :id")
    suspend fun updateNameAndTimestamp(id: String, originalFileName: String, updatedAt: Long)

    @Query("UPDATE documents SET ocrText = :ocrText WHERE id = :id")
    ...
    @Query(
        ...
        AND (:freeText IS NULL OR d.originalFileName LIKE '%' || :freeText || '%'
            OR d.ocrText LIKE '%' || :freeText || '%'
            OR dt.tag LIKE '%' || :freeText || '%'
            OR dm.value LIKE '%' || :freeText || '%')
