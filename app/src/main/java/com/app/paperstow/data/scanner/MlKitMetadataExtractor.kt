...
class MlKitMetadataExtractor @Inject constructor() : MetadataExtractor {
    ...
    internal suspend fun recognizeText(imageData: ByteArray): String {
        val bitmap = decodeForOcr(imageData)
            ?: throw ExtractionException("Unable to decode image data")

        val inputImage = InputImage.fromBitmap(bitmap, 0)
        DebugLogger.d("OCR", "recognizeText: ${bitmap.width}x${bitmap.height}")

        return suspendCancellableCoroutine { continuation ->
            textRecognizer.process(inputImage)
                .addOnSuccessListener { result ->
                    DebugLogger.i("OCR", "ML Kit text chars=${result.text.length}")
                    if (continuation.isActive) continuation.resume(result.text)
                }
                .addOnFailureListener { exception ->
                    DebugLogger.e("OCR", "ML Kit failed: ${exception.message}", exception)
                    if (continuation.isActive) continuation.resume("")
                }
        }
    }

    private fun decodeForOcr(imageData: ByteArray): android.graphics.Bitmap? {
        ...
        return BitmapFactory.decodeByteArray(imageData, 0, imageData.size, opts)
    }
