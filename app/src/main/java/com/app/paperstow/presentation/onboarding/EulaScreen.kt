 ...
 * If declined, the app closes. Acceptance is persisted with a timestamp.
...
fun EulaScreen(onAccepted: () -> Unit, onDeclined: () -> Unit) {
    Column(
        ...
        val privacyUrl = androidx.compose.ui.res.stringResource(com.app.paperstow.R.string.privacy_policy_url)
        val eulaContext = LocalContext.current
        OutlinedButton(
            onClick = {
                eulaContext.startActivity(android.content.Intent(android.content.Intent.ACTION_VIEW, android.net.Uri.parse(privacyUrl)))
            },
            modifier = Modifier.fillMaxWidth()
        ) { Text("Privacy Policy") }
...
This license does not grant you any rights to the source code, except as separately provided under the Apache 2.0 open source license governing the source repository. Third-party libraries bundled with the App (including AndroidX, Bouncy Castle, Zip4j, and Google ML Kit / Play services) remain under their own licenses and terms, which are listed in the repository NOTICE and docs/THIRD_PARTY_LICENSES.md.
        """.trimIndent(),
...
Optional My Trail: if you start it, unique places from the last 24 hours stay on this device, including battery level at each save. Locations are not uploaded. You may share Maps links yourself.
        """.trimIndent(),
...
No failure or delay by Developer in exercising any right under this Agreement shall operate as a waiver of that right.
    ...
    Text(
        "Last updated: September 2026\n© 2026 Paperstow Contributors. All rights reserved.",
        fontSize = 11.sp,
        color = Color.Gray,
        textAlign = TextAlign.Center,
        modifier = Modifier.fillMaxWidth()
