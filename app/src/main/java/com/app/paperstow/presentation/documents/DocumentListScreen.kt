...
fun DocumentListScreen(
    ...
    onSetDocList: (List<Document>) -> Unit = {},
    ...
    val visibleDocs = if (filterTag.isNullOrBlank()) documents
        else documents.filter { doc -> doc.tags.any { it.name == filterTag } }
    val filtered = !filterTag.isNullOrBlank()
        ...
        if (showingTrash) {
            ...
            if (visibleDocs.isEmpty()) {
                Column(Modifier.fillMaxSize().padding(padding), horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.Center) {
                    ...
                    Text(if (filtered) "No papers with this tag" else "Nothing stowed yet", fontSize = 18.sp, color = Color.Gray)
                    Text(if (filtered) "Tag a document with “$filterTag” to see it here" else "Import a passport, visa, or ticket to get started", fontSize = 13.sp, color = Color.LightGray)
                }
            } else {
                LazyColumn(Modifier.fillMaxSize().padding(padding).padding(horizontal = 16.dp)) {
                    ...
                    items(visibleDocs, key = { it.id }) { doc ->
                        ...
                        Card(
                            modifier = Modifier.fillMaxWidth().padding(vertical = 4.dp)
                                .combinedClickable(onClick = { if (selectionMode) viewModel.toggleSelection(doc.id) else { onSetDocList(visibleDocs); onDocumentClick(doc) } }, onLongClick = { viewModel.toggleSelection(doc.id) }),
