package com.app.paperstow.presentation.diagnostics
...
private fun shareLogsAsZip(context: android.content.Context) {
    try {
        ...
        if (!logFile.exists()) { com.app.paperstow.debug.DebugLogger.w("Diagnostics", "No log file to share"); return }
        ...
        java.util.zip.ZipOutputStream(zipFile.outputStream()).use { zip ->
            ...
            val telemetry = com.app.paperstow.debug.UsageTelemetry.getLocalSummary()
        ...
        com.app.paperstow.debug.DebugLogger.i("Diagnostics", "Logs ZIP shared (${zipFile.length() / 1024}KB)")
    } catch (e: Exception) {
        com.app.paperstow.debug.DebugLogger.e("Diagnostics", "Failed to share logs ZIP", e)
    }
}
...
private fun runDiagnostics(context: android.content.Context): Pair<String, List<String>> {
    ...
    try {
        ...
        db.close()
        com.app.paperstow.debug.DebugLogger.i("Diagnostics", report.toString())
    } catch (e: Exception) {
        issues.add("Diagnostics error: ${e.message}")
        com.app.paperstow.debug.DebugLogger.e("Diagnostics", "Failed", e)
    }
...
private fun repairDatabase(context: android.content.Context) {
    com.app.paperstow.debug.DebugLogger.i("Diagnostics", "=== REPAIR STARTED ===")
    try {
        val dbFile = context.getDatabasePath("traveldocs.db")
        if (!dbFile.exists()) {
            com.app.paperstow.debug.DebugLogger.w("Diagnostics", "No DB file — nothing to repair")
            return
        }

        // Strategy: delete the corrupt DB and let Room recreate on next access.
        // We NEVER manually CREATE TABLE for Room-managed tables because Room
        // generates exact DDL with specific column affinities and constraints
        // that manual SQL cannot replicate exactly.
        context.deleteDatabase("traveldocs.db")
        java.io.File(dbFile.path + "-wal").delete()
        java.io.File(dbFile.path + "-shm").delete()

        com.app.paperstow.debug.DebugLogger.i("Diagnostics", "Repair: deleted corrupt DB. Room will recreate tables on next query.")
        com.app.paperstow.debug.DebugLogger.i("Diagnostics", "Note: document metadata is lost but encrypted files on disk remain intact.")
        com.app.paperstow.debug.UsageTelemetry.action("Diagnostics", "repair_executed")
    } catch (e: Exception) {
        com.app.paperstow.debug.DebugLogger.e("Diagnostics", "Repair failed", e)
    }
}
